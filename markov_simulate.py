...
def next_batter_index(at_crease: Tuple[int, int]) -> int:
    """Lowest lineup index not currently at the crease (0..10)."""
    used = set(at_crease)
    for i in range(11):
        if i not in used:
            return i
...
def simulate_one_ball(
    ...
    On wicket: new batter (next in order) comes in at striker; non-striker unchanged.
    """
    ...
    if outcome == "out":
        next_batter = next_batter_index((striker_ix, non_striker_ix))
...
def simulate_innings(
    ...
    while balls < max_balls and wickets < max_wickets:
        new_striker, new_non_striker, runs, wicket, _ = simulate_one_ball(
            striker, non_striker, batter_probs, rng
        )
        total_runs += runs
        wickets += wicket
        balls += 1
